...
def mainloop():
  ...
  print("Key: F = Farmer, f = fox, C = Chicken, G = Grain")
  print("Rules: fox cannot be alone with Chicken, Chicken cannot be alone with Grain.")
  print("Goal: get all items on the right side of the river without violating the rules.")
  print("See this link for background on the puzzle: /nhttps://www.mathsisfun.com/chicken_crossing_solution.html")
  print("----------------------------------------------------------------------")
...
''')
    ...
    command = input("Enter f, C, G, or F to just move the Farmer; B-back; H-help; Q-quit. >> ")
    ...
    if command=="F":
      i = 3
    print("Operator "+str(command)+" selected.")
    if i<0 or i>= len(OPERATORS):
      print("There is no such command "+str(i))
      continue
    if applicability_vector[i]:
       ...
       print("----------------------------------------------------------------------")
       print("You cannot move the "+str(command)+" in the current state!")
       print("----------------------------------------------------------------------")
       continue
